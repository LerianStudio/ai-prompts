...
# File extensions that ESLint can check
ESLINT_EXTENSIONS = {
...
def get_eslint_command() -> str:
    """Get the appropriate ESLint command (local or global)."""
    if os.path.exists("node_modules/.bin/eslint"):
...
def format_lint_issues(issues: list) -> str:
    ...
    # Show only first few most important issues
    important_issues = []
...
def main():
    ...
    if has_errors:
        # Print to stderr but don't block with exit code 2
        print(output, file=sys.stderr)
        sys.exit(1)  # Non-blocking error
    else:
        # Print success/warning to stdout
