...
description: Apply consistent coding standards, naming conventions, and architectural patterns across the entire codebase
argument-hint: [aspect:code|naming|structure|all] [scope]
---
...
- `aspect`: What to standardize - code style, naming conventions, project structure, or all (optional, defaults to all)
- `scope`: File or directory path to standardize (optional, defaults to current directory)
...
interface FrontendNamingRules {
  ...
  components: {
    props: 'PascalCase interface' // UserProfileProps
    state: 'camelCase' // isLoading, userData
    handlers: 'handle prefix' // handleUserClick, handleSubmit
    refs: 'ref suffix' // inputRef, containerRef
  }

  hooks: {
    customHooks: 'use prefix' // useLocalStorage, useApi
    returns: 'object destructuring' // { data, loading, error }
  }

  api: {
    endpoints: 'kebab-case' // '/api/user-profile'
    queryKeys: 'array format' // ['users', userId, 'profile']
    mutations: 'verb prefix' // createUser, updateProfile
  }
}
...
async function standardizeStructure(rootPath: string) {
  ...
  for (const [dir, subdirs] of Object.entries(FRONTEND_STRUCTURE)) {
