...
class UnifiedInstaller {
  ...
  showTroubleshootingHelp(error, options = {}) {
    output.log(chalk.yellow('\n⚠ Troubleshooting Steps:'), options)
    output.log('1. Check directory permissions and disk space', options)
    output.log('2. Ensure no other installation is running', options)
    output.log(
      '3. Try running with DEBUG_INSTALLER=1 for detailed logs',
      options
    )
    output.log('4. Check Node.js version compatibility (>=16.0.0)', options)

    if (error.code) {
      output.log(chalk.gray(`\nError Code: ${error.code}`), options)
    }
  ...
  async install(options) {
    if (this.isProcessing) {
      output.warn('⚠ Installation already in progress...', options)
      return
    }

    this.isProcessing = true
    debug.log('Starting installation with options:', options)
  ...
  async showDryRun(config, options) {
    output.log(
      chalk.cyan('✓ Dry run - showing what would be installed:\n'),
      options
    )
    ...
    for (const op of operations) {
      ...
      output.log(chalk[color](`  ${symbol} ${op.path}`), options)
    }

    output.log(`\n✓ ${operations.length} operations planned`, options)
  ...
  async performInstall(config, options) {
    ...
    try {
      ...
      await this.checkDirectoryConflicts(config, spinner, options)

      // Execute the actual installation operations
      ...
      output.error(this.getActionableErrorMessage(error), options)
      this.showTroubleshootingHelp(error, options)
      process.exit(1)
    } finally {
      this.isProcessing = false
      this.currentSpinner = null
  ...
  async status(options) {
    ...
    if (!(await fs.pathExists(configPath))) {
      if (options.json) {
        ...
        output.log(chalk.gray('Run: npx lerian-protocol install'), options)
      }
      return
    }
    ...
    if (options.json) {
      ...
      output.log('✓ Location: ' + chalk.gray(cwd), options)
      output.log('✓ Version: ' + chalk.green(config.version), options)
      output.log('✓ Project: ' + chalk.blue(config.projectName), options)
      output.log(
        '✓ Installed: ' +
  ...
  async update(options) {
    if (options.check) {
      output.log(chalk.cyan('✓ Checking for updates...'), options)
      output.log(chalk.green('✓ Lerian AI Protocol is up to date!'), options)
      return
    }

    output.log(chalk.cyan('✓ Updating Lerian AI Protocol...'), options)
    // Update logic here
    output.log(chalk.green('✓ Update completed!'), options)
  }
  ...
  async uninstall(options) {
    ...
    if (!options.force) {
      ...
      if (!confirm) {
        output.log('Uninstall cancelled', options)
        return
      }
    }
    ...
    for (const target of targets) {
      ...
      if (await fs.pathExists(targetPath)) {
        ...
        output.success(`Removed ${target}`, options)
      }
    }

    output.success('Lerian AI Protocol uninstalled successfully', options)
  }
  ...
  async confirmInstallation(config, spinner, options) {
    ...
    if (!proceed) {
      output.warn('Installation cancelled', options)
      process.exit(0)
    }
  ...
  async checkDirectoryConflicts(config, spinner, options) {
