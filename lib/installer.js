...
class UnifiedInstaller {
  ...
  /**
   * Dynamically discovers all directories that need to be created in the target
   */
  async discoverDirectoryStructure() {
    debug.log('Discovering directory structure from source...')
    const directories = [...DEFAULTS.coreDirectories]
    
    // Discover .claude subdirectories
    await this.discoverClaudeStructure(directories)
    
    // Discover protocol-assets subdirectories  
    await this.discoverProtocolAssetsStructure(directories)
    
    debug.log(`Discovered ${directories.length} directories: ${directories.join(', ')}`)
    return directories
  }

  async discoverClaudeStructure(directories) {
    const claudeSource = path.join(this.sourceRoot, '.claude')
    
    if (!(await fs.pathExists(claudeSource))) {
      debug.log('No .claude source directory found')
      return
    }

    await this.walkDirectory(claudeSource, (relativePath) => {
      const targetPath = path.join('.claude', relativePath)
  ...
  async discoverProtocolAssetsStructure(directories) {
    const protocolAssetsSource = path.join(this.sourceRoot, 'protocol-assets')
    
    if (!(await fs.pathExists(protocolAssetsSource))) {
      debug.log('No protocol-assets source directory found')
      return
    }

    await this.walkDirectory(protocolAssetsSource, (relativePath) => {
      const targetPath = path.join('protocol-assets', relativePath)
      if (!directories.includes(targetPath)) {
        directories.push(targetPath)
      }
    })
  ...
  /**
   * Recursively walks a directory and calls callback for each directory found
   */
  async walkDirectory(sourceDir, callback, relativePath = '') {
    try {
      ...
      debug.error(`Error walking directory ${sourceDir}`, error)
    }
  }
  ...
  async getBoardDirectories() {
    ...
    if (!(await fs.pathExists(boardSource))) {
      ...
      return [
        ...
        'protocol-assets/board/01.backlog',
        'protocol-assets/board/02.ready',
        'protocol-assets/board/03.in-progress',
        'protocol-assets/board/04.testing',
        'protocol-assets/board/05.completed'
      ]
    }

    try {
      const boardDirs = ['protocol-assets/board']
      const entries = await fs.readdir(boardSource, { withFileTypes: true })

      for (const entry of entries) {
        if (entry.isDirectory() && !entry.name.startsWith('.')) {
          boardDirs.push(path.join('protocol-assets/board', entry.name))
        }
      }
  ...
  async planInstallation(config) {
    ...
    // Dynamically discover all directories that need to be created
    const directories = await this.discoverDirectoryStructure()
    ...
    operations.push(
      {
        type: 'create',
        path: path.join(config.directory, '.claude'),
        description: ' Installing Claude Code configuration',
        operation: 'installClaude',
        retryable: true
      },
      {
        type: 'create',
        path: path.join(config.directory, 'protocol-assets'),
        description: ' Installing protocol assets and resources',
      ...
      },
      {
        ...
        description: ' Creating .mcp.json configuration',
  ...
  async executeOperation(operation, config) {
    switch (operation.operation) {
      ...
      case 'installClaude':
        await this.installClaude(config.directory, config)
        return { action: 'created' }

      case 'installProtocolAssets':
        await this.installProtocolAssets(config.directory)
        return { action: 'created' }
  ...
  async installContext(installDir) {
    debug.log('Context installation handled by protocol-assets structure copy')
    return
  }

  async installClaude(installDir, config) {
    const claudeSource = path.join(this.sourceRoot, '.claude')
    const claudeDest = path.join(installDir, '.claude')

    if (path.resolve(claudeSource) === path.resolve(claudeDest)) {
      debug.log('Claude source and destination are the same, skipping copy')
      return
    }

    if (await fs.pathExists(claudeSource)) {
      await fs.copy(claudeSource, claudeDest)
      debug.log(`Copied .claude from ${claudeSource} to ${claudeDest}`)
      
      // Make sure hook files are executable
      const hooksDir = path.join(claudeDest, 'hooks')
      if (await fs.pathExists(hooksDir)) {
      ...
      debug.log('No .claude source directory found, creating minimal structure')
      await fs.ensureDir(claudeDest)
      await this.createClaudeMd(installDir, config)
    }
  }

  async installProtocolAssets(installDir) {
    const protocolAssetsSource = path.join(this.sourceRoot, 'protocol-assets')
    ...
    if (await fs.pathExists(protocolAssetsSource)) {
      await fs.copy(protocolAssetsSource, protocolAssetsDest)
      debug.log(`Copied protocol-assets from ${protocolAssetsSource} to ${protocolAssetsDest}`)
    } else {
      debug.log('No protocol-assets source directory found, skipping copy')
    }
  }
  ...
  async updateGitignore(installDir) {
    ...
    try {
      ...
      // Check if we need to update existing Lerian Protocol entries
      if (existingContent.includes('# Lerian Protocol')) {
        if (existingContent.includes('.workflow/')) {
          debug.log('Updating existing Lerian Protocol entries with new structure')
          // Replace old .workflow/ with protocol-assets/
          existingContent = existingContent.replace('.workflow/', 'protocol-assets/')
          await fs.writeFile(gitignorePath, existingContent, 'utf8')
          debug.log('Successfully updated .gitignore to use protocol-assets/')
          return
        } else {
          debug.log('.gitignore already contains correct Lerian Protocol entries')
          return
        }
      }
