...
class Installer {
  ...
  async getBoardDirectories() {
    const boardSource = path.join(this.sourceRoot, 'protocol-assets', 'shared', 'board')
    const destinationPrefix = 'protocol-assets/shared/board'

    if (!(await fs.pathExists(boardSource))) {
      debug.log('Board source not found, skipping board directory creation')
      return []
    }
