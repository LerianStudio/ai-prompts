...
class Installer {
  ...
  shouldIncludeFile(filePath, profile, baseDir) {
    ...
    // Get the base directory name to determine which patterns apply
    const baseDirName = path.basename(baseDir)

    const matches = patterns.some((pattern) => {
      // Handle different base directories
      {
        // For .claude directories and legacy protocol-assets
  ...
  showNextSteps(config, spinner) {
    ...
    examplesTable.push(
      ...
      [
        theme.info('Remote Services'),
        theme.muted('MCP servers (external)'),
    ...
    filesTable.push(
      ...
      [
        ...
        'Frontend/backend assets and templates'
      ],
      [
        theme.accent('Config'),
  ...
  async getBoardDirectories() {
    // Board system directories are no longer created in target projects
    return []
  }
  ...
  filterDirectoriesByProfile(directories, profile) {
    ...
    return directories.filter((dir) => {
      ...
      // Handle legacy protocol-assets directory
  ...
  async installProtocolAssets(installDir, spinner, profile = 'full') {
    // Copy legacy protocol-assets directory if it exists
