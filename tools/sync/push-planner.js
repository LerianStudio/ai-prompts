...
class PushPlanner {
  ...
  async createPushPlan(
    ...
    const targetChanges = this.fileHasher.compareHashes(
      lastPushHashes,
      pushableFiles
    )
    const sourceChanges = this.fileHasher.compareHashes(
      lastPushHashes,
    ...
    if (await fs.pathExists(packageJsonPath)) {
      try {
        ...
        if (packageJson.name !== 'lerian-protocol') {
          errors.push(
            'Source directory does not appear to be a Lerian Protocol package'
          )
        }
      } catch (error) {
  ...
  async getBaselineChanges(targetPath, pushableFileHashes) {
    try {
      const changeResult = await this.baselineManager.detectChanges(
        targetPath,
        pushableFileHashes
      )
      return changeResult
      ...
      return {
        hasBaseline: false,
        changes: {
          added: Object.keys(pushableFileHashes).map((path) => ({
            path,
            ...pushableFileHashes[path]
          })),
          modified: [],
          deleted: [],
          unchanged: []
  ...
  async getGitChangedFiles(targetPath, allFileHashes = {}) {
    ...
    try {
      ...
      for (const [filePath, fileInfo] of Object.entries(allFileHashes)) {
        if (
          this.matchesAnyPattern(filePath, pushablePatterns) &&
          this.isRecentlyModified(fileInfo)
        ) {
          changedFiles.add(filePath)
        }
      }
  ...
  async filterPushableFiles(
    ...
    if (!hasExistingMetadata && targetPath && !selection.showAll) {
      ...
      for (const [filePath, fileInfo] of Object.entries(targetHashes)) {
        if (
          this.matchesAnyPattern(filePath, patterns) &&
          this.isPushSafe(filePath)
        ) {
          candidateFiles[filePath] = fileInfo
        }
      }

      baselineChanges = await this.getBaselineChanges(
        targetPath,
        candidateFiles
      )
      ...
      if (!baselineChanges.hasBaseline) {
        console.log(
          `📝 No push baseline found. Creating baseline with ${baselineChanges.summary.total} eligible files.`
        )
        console.log(
          `   Use --all to push all files, or select specific files interactively.`
        )
        ...
        const changedCount =
          baselineChanges.summary.added + baselineChanges.summary.modified
        if (changedCount > 0) {
          console.log(
            `🔍 Baseline change detection: ${changedCount} changed files found`
          )
          console.log(
            `   Added: ${baselineChanges.summary.added}, Modified: ${baselineChanges.summary.modified}`
          )
        } else {
          console.log(
            `🔍 No changes since last push (use --all to show all eligible files)`
          )
          return {}
        }
      }
      ...
      if (this.matchesAnyPattern(filePath, patterns)) {
        ...
        if (this.isPushSafe(filePath)) {
          ...
          if (useBaselineFiltering && baselineChanges) {
            ...
            const isChanged =
              baselineChanges.changes.added.some((f) => f.path === filePath) ||
  ...
  isPushSafe(filePath) {
    ...
    if (filePath.startsWith('.claude/')) {
      ...
      return !restrictedClaudeFiles.some(
        (restricted) => filePath === restricted
      )
    }
  ...
  wasFileChanged(filePath, changes) {
    return (
      changes.added.some((f) => f.path === filePath) ||
  ...
  isCriticalFile(filePath) {
    const criticalPatterns = ['package.json', 'bin/**', 'lib/**/*.js']
  ...
  isSystemFile(filePath) {
    const systemPatterns = ['.git/**', 'node_modules/**', '.env', '*.lock']
