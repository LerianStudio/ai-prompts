...
class Installer {
  ...
  async getInteractiveConfig(directory, options) {
    ...
    output.log(theme.muted('Choose the components you need for your project:\n'))
  ...
  showNextSteps(config, spinner) {
    ...
    filesTable.push(
      ...
      [
        ...
        theme.muted('protocol-assets/lib/board-mcp/'),
        'Task management MCP server'
      ],
      [
        theme.accent('Board Service'),
        theme.muted('protocol-assets/lib/board-service/'),
  ...
  async planInstallation(config) {
    ...
    const directories = this.filterDirectoriesByProfile(allDirectories, config.profile || 'full')
  ...
  async getInstallationOperations(config) {
    ...
    operations.push(
      ...
      {
        type: 'create',
        path: path.join(config.directory, 'protocol-assets/lib/board-mcp'),
      ...
      {
        type: 'create',
        path: path.join(config.directory, 'protocol-assets/lib/board-service'),
        description: ' Installing board service',
      ...
      {
        type: 'create',
        path: path.join(config.directory, 'protocol-assets/data/databases/task-management.db'),
  ...
  async performInstallation(config, options) {
    ...
    try {
      ...
      await this.createInitialPushBaseline(config.directory, config.profile, installSpinner)
  ...
  async executeOperation(operation, config, spinner) {
    switch (operation.operation) {
      ...
      case 'installProtocolAssets':
        await this.installProtocolAssets(config.directory, spinner, config.profile)
        return { action: 'created' }
  ...
  async confirmInstallation(config, spinner, options) {
    ...
    const profileInfo = config.profile === 'full' ? 'all profiles' : `${config.profile} profile`
  ...
  async validateNodeVersion(spinner) {
    try {
      ...
      const { stdout } = await require('util').promisify(require('child_process').exec)('node --version')
  ...
  async installAgents(installDir, spinner, profile = 'full') {
    ...
    if (await fs.pathExists(agentsSource)) {
      const profileFilter = (src) => {
        return this.shouldIncludeFile(src, profile, agentsSource) &&
  ...
  async installCommands(installDir, spinner, profile = 'full') {
    ...
    if (await fs.pathExists(commandsSource)) {
      const profileFilter = (src) => {
        return this.shouldIncludeFile(src, profile, commandsSource) &&
               !src.includes('node_modules') && 
               !src.includes('.git')
      }
  ...
  async installHooks(installDir, spinner, profile = 'full') {
    ...
    if (await fs.pathExists(hooksSource)) {
      const profileFilter = (src) => {
        return this.shouldIncludeFile(src, profile, hooksSource) &&
               !src.includes('node_modules') && 
               !src.includes('.git')
      }
  ...
  async createMetadata(installDir, spinner, profile = 'full') {
    return this.executeWithRetry(
      async () => {
        ...
        await metadataManager.createMetadata(installDir, this.sourceRoot, { profile })
  ...
  async installProtocolAssets(installDir, spinner, profile = 'full') {
    ...
    if (await fs.pathExists(protocolAssetsSource)) {
      const profileFilter = (src, _dest) => {
        return this.shouldIncludeFile(src, profile, protocolAssetsSource) &&
               !src.includes('node_modules') && 
               !src.includes('.git')
      }
      ...
      debug.log(`Copied protocol-assets (${profile} profile) from ${protocolAssetsSource} to ${protocolAssetsDest}`)
    } else {
    ...
    if (profile === 'frontend' || profile === 'full') {
      hooks.PostToolUse.push({
        ...
        "hooks": [
          ...
          {
            "type": "command",
            "command": "$CLAUDE_PROJECT_DIR/.claude/hooks/frontend/dependency-monitor.py"
          },
          {
            "type": "command",
  ...
  async installSettings(installDir, spinner, profile = 'full') {
    return this.executeWithRetry(
      async () => {
        ...
        debug.log(`Installing settings.json with profile ${profile} at: ${settingsPath}`)
        ...
        const profileSettings = await this.customizeSettingsForProfile(baseSettings, profile)
  ...
  async installBoardMcp(installDir, spinner) {
    const boardMcpSource = path.join(this.sourceRoot, 'protocol-assets', 'lib', 'board-mcp')
    const boardMcpDest = path.join(installDir, 'protocol-assets', 'lib', 'board-mcp')
    ...
    if (await fs.pathExists(boardMcpSource)) {
      ...
      await this.copyWithFileProgress(boardMcpSource, boardMcpDest, 'board MCP file', spinner, {
  ...
  async installBoardService(installDir, spinner) {
    const boardServiceSource = path.join(this.sourceRoot, 'protocol-assets', 'lib', 'board-service')
    const boardServiceDest = path.join(installDir, 'protocol-assets', 'lib', 'board-service')

    if (path.resolve(boardServiceSource) === path.resolve(boardServiceDest)) {
      debug.log('Board service source and destination are the same, skipping copy')
      return
    }

    if (await fs.pathExists(boardServiceSource)) {
      spinner.text = 'Installing... board service'
      await this.copyWithFileProgress(boardServiceSource, boardServiceDest, 'board service file', spinner, {
        filter: (src) => !src.includes('node_modules') && !src.includes('.git') && !src.includes('dist')
      })
      debug.log(`Copied board service from ${boardServiceSource} to ${boardServiceDest}`)
    }
  }

  async installBoardScripts(installDir, spinner) {
    const scriptsSource = path.join(this.sourceRoot, 'protocol-assets', 'scripts')
    const scriptsDest = path.join(installDir, 'protocol-assets', 'scripts')
  ...
  async initializeBoardDatabase(installDir, spinner) {
    return this.executeWithRetry(
      async () => {
        const dbPath = path.join(installDir, 'protocol-assets', 'data', 'databases', 'task-management.db')
    ...
    try {
      ...
      const fileHasher = new FileHasher({
        ignorePatterns: [
          ...
          '.sync-backup', '.push-backup', '*.tmp', 'dist', 'build',
      ...
      const pushableFiles = {}
      const defaultPushablePatterns = ['.claude/**', 'protocol-assets/**', 'docs/**/*.md', 'README*.md']
      
      for (const [filePath, fileInfo] of Object.entries(allFiles)) {
        if (pushPlanner.matchesAnyPattern(filePath, defaultPushablePatterns) && 
            pushPlanner.isPushSafe(filePath)) {
          pushableFiles[filePath] = fileInfo
        }
      }
      
      debug.log(`Creating baseline with ${Object.keys(pushableFiles).length} pushable files`)
      ...
      if (spinner) {
        ...
        console.warn(chalk.yellow('⚠️  Push baseline creation failed - push command will create it on first use'))
      }
