...
// Load environment variables from project root .env.dev
...
if (result.error) {
  console.error('❌ Failed to load .env.dev:', result.error.message);
  console.error('Looking for .env.dev at:', envPath);
  process.exit(1);
}
...
// Configuration from environment variables - no fallbacks, must be set in .env
const CONFIG = {
  boardApiUrl: process.env.BOARD_API_URL,
  mcpPort: parseInt(process.env.MCP_SERVER_PORT),
  mcpHost: process.env.HOST,
  healthPort: parseInt(process.env.MCP_HEALTH_PORT)
};
...
async function main() {
  try {
    ...
    // Validate configuration - all environment variables are required
