...
export class ClaudeCliManager extends EventEmitter {
  ...
  async createSession(sessionId, taskId) {
    ...
    const session = {
      ...
      timeouts: new Set(), // Track all active timeouts for cleanup
      subscriptionStatus: {
        authenticated: false,
        hasWarnings: false,
        lastWarning: null
  ...
  async sendPrompt(session, prompt, taskId) {
    return new Promise((resolve, reject) => {
      // Enhanced response tracking with timeout management
      const responseId = `resp_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

      session.pendingResponse = {
        id: responseId,
        resolve: (result) => {
          this.clearSessionTimeout(session, timeoutId);
          resolve(result);
        },
        reject: (error) => {
          this.clearSessionTimeout(session, timeoutId);
      ...
      try {
        ...
        // Set timeout for response with proper cleanup tracking
        const timeoutId = setTimeout(() => {
          if (session.pendingResponse && session.pendingResponse.id === responseId) {
            this.logger.warn('Claude CLI response timeout', {
              sessionId: session.id,
              taskId,
              responseId,
            ...
            const timeoutError = new Error(`Claude CLI response timeout after ${this.config.timeout}ms`);
            session.pendingResponse.reject(timeoutError);
            session.pendingResponse = null;

            // Remove from timeout tracking
            session.timeouts.delete(timeoutId);
          }
        }, this.config.timeout);

        // Track timeout for cleanup
        session.timeouts.add(timeoutId);

        this.logger.debug('Response timeout set', {
          sessionId: session.id,
          responseId,
          timeoutId: timeoutId.toString(),
          activeTimeouts: session.timeouts.size
        });

      } catch (error) {
        this.logger.error('Failed to send prompt to Claude CLI', {
          sessionId: session.id,
          taskId,
          error: error.message
        });
  ...
  /**
   * Clear a specific timeout from session tracking
   */
  clearSessionTimeout(session, timeoutId) {
    if (timeoutId && session.timeouts.has(timeoutId)) {
      clearTimeout(timeoutId);
      session.timeouts.delete(timeoutId);

      this.logger.debug('Cleared session timeout', {
        sessionId: session.id,
        timeoutId: timeoutId.toString(),
        remainingTimeouts: session.timeouts.size
      });
    }
  }
   ...
   */
  clearAllSessionTimeouts(session) {
    if (session.timeouts && session.timeouts.size > 0) {
      this.logger.debug('Clearing all session timeouts', {
        sessionId: session.id,
        timeoutCount: session.timeouts.size
      });

      for (const timeoutId of session.timeouts) {
        clearTimeout(timeoutId);
      }
  ...
  /**
   * Kill a specific session with proper cleanup
   */
  killSession(sessionId) {
    const session = this.activeSessions.get(sessionId);
    if (session) {
      this.logger.info('Killing Claude CLI session', {
        sessionId,
        activeTimeouts: session.timeouts ? session.timeouts.size : 0
      });

      // Clear all pending timeouts to prevent memory leaks
      this.clearAllSessionTimeouts(session);

      // Clear pending response
      if (session.pendingResponse) {
        session.pendingResponse.reject(new Error('Session terminated'));
        session.pendingResponse = null;
      }
