...
// Load environment variables from protocol-assets .env.dev
...
class TaskManagementServer {
  ...
  setupWebSocket() {
    this.wss.on('connection', (ws, req) => {
      ...
      this.wsMetrics.peakConnections = Math.max(this.wsMetrics.peakConnections, this.clients.size + 1);
      this.wsMetrics.lastActivity = Date.now();

      this.logger.info('WebSocket client connected', {
        remoteAddress: req.socket.remoteAddress,
        userAgent: req.headers['user-agent'],
        totalClients: this.clients.size + 1,
        totalConnections: this.wsMetrics.totalConnections,
      ...
      ws.on('close', (code, reason) => {
        ...
        this.wsMetrics.totalDisconnections++;
      ...
      ws.on('error', (error) => {
        this.wsMetrics.totalErrors++;
        this.logger.error('WebSocket error', {
          error: error.message,
          code: error.code,
          connectionDuration: Date.now() - ws.connectedAt,
          messageCount: ws.messageCount
        });
        this.clients.delete(ws);
      });
  ...
  setupRoutes() {
    ...
    // WebSocket health and metrics endpoint
    this.app.get('/health/websocket', (req, res) => {
      const now = Date.now();
      const activeConnections = Array.from(this.clients).map(ws => ({
        connectedAt: ws.connectedAt,
        lastActivity: ws.lastActivity,
        messageCount: ws.messageCount,
        connectionDuration: now - ws.connectedAt,
        idleTime: now - ws.lastActivity,
