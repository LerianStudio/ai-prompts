...
if [ "$1" = "--dev" ]; then
    ...
    mkdir -p "$PROJECT_ROOT/protocol-assets/infrastructure/data/pids"
    echo $! > "$PROJECT_ROOT/protocol-assets/infrastructure/data/pids/task-service.pid"
    echo "✅ Service started in background (PID: $(cat "$PROJECT_ROOT/protocol-assets/infrastructure/data/pids/task-service.pid"))"
