...
DEFAULT_DB_PATH="$PROJECT_ROOT/protocol-assets/data/databases/task-management.db"

PORT=${TASK_SERVICE_PORT:-$DEFAULT_PORT}
...
if lsof -Pi :$PORT -sTCP:LISTEN -t >/dev/null ; then
    ...
    echo "   Another service may be running. Use TASK_SERVICE_PORT to specify a different port."
    echo "   Example: TASK_SERVICE_PORT=3003 ./scripts/start-task-service.sh"
    exit 1
fi
...
if [ "$1" = "--dev" ]; then
    ...
    mkdir -p "$PROJECT_ROOT/protocol-assets/data/pids"
    echo $! > "$PROJECT_ROOT/protocol-assets/data/pids/task-service.pid"
    echo "✅ Service started in background (PID: $(cat "$PROJECT_ROOT/protocol-assets/data/pids/task-service.pid"))"
